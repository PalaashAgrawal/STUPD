...
all_prepositions = {i:k for k,i in enumerate(dynamic_prepositions)}
...
def process_csv_file(file_path, dataset_parent_path):
    ...
    for _, row in df.iterrows():
        ...
        img_idxs = [0, 14, 29] #choose first, middle and last frame from 30 frames
        image_path = [os.path.join(dataset_parent_path, eval(row["image_path"])[i]) for i in img_idxs] #choose middle frame from 30 frames
...
def zero_shot_classification(image_paths, candidate_labels, target_label):
    ...
    for image_path, labels in (zip(image_paths, candidate_labels)):
        ...
        image = torch.stack([preprocess(Image.open(img)).to(device) for img in image_path])
        ...
        #pass an averaged out image 
        with torch.no_grad():
            image_features = model.encode_image(image).mean(dim=0, keepdim=True)
...
results = {}
for preposition_file in all_prepositions:
