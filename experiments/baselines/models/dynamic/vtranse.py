...
class avg_accross_channels(nn.Module):
    def __init__(self):
        super().__init__()
...
class VtransE(vtranse_utils):
    def __init__(self,
        ...
        self.backbone = nn.Sequential(
            ...
            self.backbone.layer4, #putput of channels N,512,H,W; after this we squeeze
            avg_accross_channels(),
        ...
        self.linear2 = nn.Linear(
            # visual_feature_size * visual_feature_size * 512,
            #PAg: had to change this dimension
            visual_feature_size*visual_feature_size,
            visual_feature_size * visual_feature_size * 64,
        )
    ...
    def forward(self, subj, obj, full_im, t_s, t_o, bbox_s, bbox_o):
        ...
        subj_img_feature = torch.cat(subj_img_feature).squeeze(dim =1)
        obj_img_feature = torch.cat(obj_img_feature).squeeze(dim=1)
