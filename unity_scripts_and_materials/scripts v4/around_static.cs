...
public class around_static : Randomizer
{
    ...
    protected override void OnIterationStart()
    {
        ...
        currentInstance2 = GameObject.Instantiate(prefabs2.Sample(), pos1, Quaternion.identity);
        ...
        currentInstance1 = GameObject.Instantiate(prefabs1.Sample(), new Vector3(1000,0,0),Quaternion.identity);
        ...
        for (int loop = 0; loop < objectCount; loop++)
        {
            ...
            Vector3 spawnPoint = currentInstance2.transform.position + circle3D;
            ...
            GameObject spawnedObject = GameObject.Instantiate(currentInstance1, offsetSpawnPoint, Quaternion.identity);
        ...
        {
            if (GameObjects[i].name==currentInstance2.name || GameObjects[i].name==currentInstance2.name+"(Clone)") 
            {
            ...
            else if (GameObjects[i].name==currentInstance1.name || GameObjects[i].name==currentInstance1.name+"(Clone)") 
            {
                if (!GameObjects[i].name.StartsWith("person") && !GameObjects[i].name.StartsWith("track"))
                {
    ...
    protected override void OnIterationEnd()
    {
        ...
        for (int i = 0; i < GameObjects.Length; i++)
        {
            if (GameObjects[i].name==currentInstance2.name || GameObjects[i].name==currentInstance2.name+"(Clone)" || GameObjects[i].name==currentInstance1.name || GameObjects[i].name==currentInstance1.name+"(Clone)")
            {
